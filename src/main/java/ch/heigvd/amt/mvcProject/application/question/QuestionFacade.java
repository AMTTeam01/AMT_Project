...
public class QuestionFacade {
    ...
    private QuestionsDTO.QuestionDTO getQuestionAsDTO(Question question, Collection<AnswersDTO.AnswerDTO> answers,
        ...
        QuestionsDTO.QuestionDTO.QuestionDTOBuilder builder = QuestionsDTO.QuestionDTO.builder()
                ...
                .username(userFacade.getUsers(UserQuery.builder()
                        .userId(question.getUserId()).build()).getUsers().get(0).getUsername())
    ...
    private Collection<AnswersDTO.AnswerDTO> getAnswers(Question question) {
        return question.getAnswers().stream().map(
                answer -> {
                    Collection<CommentsDTO.CommentDTO> commentsDTO = new ArrayList<>();
                    AnswersDTO.AnswerDTO build = null;
                    try {
                        commentsDTO = commentFacade.getComments(CommentQuery.builder().answerId(answer.getId()).build())
                                        .getComments();

                        build = AnswersDTO.AnswerDTO.builder()
                                .id(answer.getId())
                                .creationDate(answer.getCreationDate())
                                .description(answer.getDescription())
                                .username(userFacade.getUsers(UserQuery.builder().userId(answer.getUserId()).build()).getUsers().get(0).getUsername())
